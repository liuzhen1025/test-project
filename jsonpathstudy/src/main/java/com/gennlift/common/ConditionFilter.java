...
public class ConditionFilter {
    ...
    public void splitCondition(String source, String regx, List<String> result){
        ...
        if(getOrIndexNotInBraces(forOrInSource)){
            result.add(forOrInSource);
            splitCondition(source.substring(orFirstIndex+regx.length(),source.length()), regx,result);
        }else{
            int temp = findPatterIndex(source,regx);
            String tempR = source.substring(orFirstIndex,source.length());
            int firstRightBrace = findFirstRightBrace(tempR)+1;
            result.add(source.substring(0,orFirstIndex+firstRightBrace));
            splitCondition(source.substring(orFirstIndex+firstRightBrace,source.length()), regx,result);
        }
    }

    public int findFirstRightBrace(String source){
        int length = StringUtils.isEmpty(source)? 0:source.length();
     ...
     * @return
     */
    public boolean getOrIndexNotInBraces(String source){
        return isInBraces(source );
    }
    ...
    public String manySpaceToOneSpace(String resource){
        return StringUtils.replaceAll(resource,"\\s+"," ");
    }
}
